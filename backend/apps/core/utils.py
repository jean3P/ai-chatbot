...
def chunk_text(text: str, max_size: int = 1200, overlap: int = 200) -> List[str]:
    ...
    while start < len(text):
        ...
        if break_point > start + max_size // 2:  # Only break if we're not too close to start
...
def format_conversation_context(messages: List[Dict[str, Any]], max_context_length: int = 2000) -> str:
