...
class VectorStore:
    ...
    def add_vectors(self, chunk_ids: List[str], vectors: List[List[float]], metadata: List[Dict] = None):
    ...
    def search(self, query_vector: List[float], top_k: int = 10, min_score: float = 0.0) -> List[Dict]:
...
def load_embeddings_to_memory() -> VectorStore:
    ...
    try:
        ...
        chunks = DocumentChunk.objects.select_related('document').exclude(embedding__isnull=True)
...
def calculate_chunk_similarity(chunk1_embedding: List[float], chunk2_embedding: List[float]) -> float:
...
def find_duplicate_chunks(similarity_threshold: float = 0.95) -> List[Tuple[str, str, float]]:
    """Find potentially duplicate chunks based on embedding similarity"""
    duplicates = []

    try:
        chunks = list(DocumentChunk.objects.exclude(embedding__isnull=True).values('id', 'embedding'))

        for i, chunk1 in enumerate(chunks):
            for chunk2 in chunks[i+1:]:
                similarity = calculate_chunk_similarity(chunk1['embedding'], chunk2['embedding'])
    ...
    try:
        ...
        if duplicates:
            ...
            for chunk1_id, chunk2_id, similarity in duplicates[:10]:  # Log first 10
                logger.warning(f"Chunks {chunk1_id} and {chunk2_id} have similarity {similarity:.3f}")
...
def validate_embeddings() -> Dict[str, Any]:
    ...
    try:
        ...
        if len(results['dimensions_found']) > 1:
            ...
            logger.warning(f"Inconsistent embedding dimensions found: {results['dimensions_found']}")
