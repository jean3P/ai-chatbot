...
class Command(BaseCommand):
    help = 'Batch upload documents from directory structure with automatic categorization'
    ...
    def handle(self, *args, **options):
        ...
        if not pdf_files:
            self.stdout.write(self.style.WARNING('No PDF files found in the directory structure'))
            return
    ...
    def extract_document_info(self, filename, product_info):
        ...
        document_type = 'other'
        if any(word in filename_lower for word in ['manual', 'bedienungsanleitung', 'handbuch']):
            document_type = 'manual'
        elif any(word in filename_lower for word in ['datasheet', 'datenblatt', 'spec']):
        ...
        # Create title
        title = f"{product_info['product_model']} - {filename_clean}" if product_info[
            'product_model'] else filename_clean
