...
def test_chunk_data():
    ...
    if chunks_with_embeddings > 0:
        ...
        print(f"Sample embedding dimension: {len(sample_chunk.embedding) if sample_chunk.embedding else 0}")
        if sample_chunk.metadata:
            print(f"Sample embedding model: {sample_chunk.metadata.get('embedding_model', 'unknown')}")
    ...
    for query in test_queries:
        try:
            ...
            for i, result in enumerate(results):
                ...
                print(f"  {i + 1}. Score: {score:.3f} - {doc_title} (p.{page}) - {content}...")
    ...
    try:
        ...
        if response['sources']:
            ...
            for i, source in enumerate(response['sources']):
                print(f"  {i + 1}. {source['document']} (p.{source['page']}) - Score: {source['similarity']:.3f}")
